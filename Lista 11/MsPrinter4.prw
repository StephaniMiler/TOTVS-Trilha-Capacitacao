...
Static Function ImpDados(cAlias)
    ...
    while (cAlias)->(!EOF())
        ...
        oPrint:Say(nLinha, 120, AllTrim((cAlias)->(C7_DESCRI)), oFont10,, nCor)   
        // Imprimindo qtde vendida
        oPrint:Say(nLinha, 270, + CVALTOCHAR((cAlias)->(C7_QUANT)), oFont10,, nCor)
        // Imprimindo valor unitário
        oPrint:Say(nLinha, 380, CVALTOCHAR((cAlias)->(C7_PRECO)), oFont10,, nCor)
        // Imprimindo valor total
        oPrint:Say(nLinha, 450, CVALTOCHAR((cAlias)->(C7_TOTAL)), oFont10,, nCor) 
        // Faz a soma de todos os valores da coluna 'TOTAL'
        oPrint:Say(nLinha, 450, CVALTOCHAR( 'C7_TOTAL' ), 'VALTOT', 'SUM', oPrint, 'VALOR TOTAL' ) 
        ...
        (cALias)->(DbSkip())
        If cPed != AllTrim((cAlias)->(C7_NUM))
            oPrint:Line(700, 015, 700, 580, /*Cor*/, '-6')
            oPrint:Say(720, 450, 'Total R$' + CVALTOCHAR(nTotalizador), oFont10,, VERMELHO)
        Endif
