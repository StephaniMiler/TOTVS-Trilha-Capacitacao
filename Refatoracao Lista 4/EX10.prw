...
#INCLUDE 'PROTHEUS.CH'
#DEFINE TITULO_TELA 'Folha de pagamento'

/*/{Protheus.doc} User Function Holerite
    Calcula e imprime descontos salariais
    @type  Function
    @author Stephani Miler
    @since 13/03/2023
/*/
User Function Holerite() 
    ...
    FwAlertInfo('Sal·rio bruto (' + ALLTRIM(STR(nHora)) + '*' + ALLTRIM(STR(nQuantidadeHoras)) + ')_______: R$ ' ;
    + ALLTRIM(StrZero(nSalarioBruto,7,2)) + CRLF + '( - ) IR (' + ALLTRIM(cIR) + ')_______________: R$ ' ;
    + ALLTRIM(StrZero(nIR,7,2)) + CRLF + '( - ) INSS (10%)____________: R$ ' + ALLTRIM(StrZero(nINSS,7,2)) ;
    + CRLF + 'FGTS (11%)_______________: R$ ' + ALLTRIM(StrZero(nFGTS,7,2)) + CRLF + ;
