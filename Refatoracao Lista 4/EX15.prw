...
#DEFINE TEXTO 'Insira o código do produto'

/*/{Protheus.doc} User Function FindProd
    pergunta se o usuário deseja cadastrar um novo produto, caso o mesmo não esteja cadastrado
    @type  Function
    @author Stephani Miler
    @since 13/03/2023
/*/
User Function FindProd()
    ...
    @ 010, 160 BUTTON 'Buscar'   SIZE 55, 11 OF oDlg PIXEL ACTION BuscaProduto(cCodigo)
...
Static Function BuscaProduto(cCodigo)
...
Static Function InserirProduto()
    ...
    Local cCodigo := Space(15), cDesc := Space(50), cTip := Space(10), cUni := Space(10), cArm := Space(10)
    Local nOpcao := 0
    Local oDlg := NIL
    ...
    @ 024, 010 MSGET cCodigo SIZE 55, 11 OF oDlg PIXEL
    ...
    @ 010, 700 BUTTON 'Salvar'   SIZE 45, 11 OF oDlg PIXEL ACTION ValidaProd(cCodigo, cDesc, cTip, cUni, cArm)
    @ 010, 650 BUTTON 'Cancelar' SIZE 45, 11 OF oDlg PIXEL ACTION (nOpcao := 2, oDlg:End())
    ACTIVATE MSDIALOG oDlg CENTERED

Return

Static Function ValidaProd(cCodigo, cDesc, cTip, cUni, cArm)

    if ALLTRIM(cCodigo) != ''.AND. ALLTRIM(cDesc) != '' .AND. ALLTRIM(cTip) != ''.AND. ALLTRIM(cUni) != '' .AND. ALLTRIM(cArm) != ''
